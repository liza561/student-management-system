...
def student_home(request):
    ...
    for sub in subjects:
        present_count = AttendanceReport.objects.filter(student_id=student_obj,attendance_id__subject_id=sub,status=True).count()
        absent_count = AttendanceReport.objects.filter(student_id=student_obj,attendance_id__subject_id=sub,status=False).count()

        present_data.append(present_count)
        absent_data.append(absent_count)

    return render(request, "student_template/student_home_template.html", {"total_attendance": attendance_total,"attendance_present": attendance_present,"attendance_absent": attendance_absent,"subjects": subjects,"subject_labels": json.dumps(subject_labels),"data1": json.dumps(present_data),"data2": json.dumps(absent_data),
    })
